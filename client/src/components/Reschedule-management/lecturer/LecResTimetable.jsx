...
import LecturerRequestForm from "./LecturerRequest";
import axios from "axios";
import "../css/theme.css"


function LecResTimetable({selectedModule }) {
  ...
  const handleCellClick = (subject, day, time) => {
    if (!subject) {
      setSelectedCellData({ subject, day, time });
    } else {
    }
  };
  ...
  const renderCell = (day, time) => {
    ...
    return subject ? subject : null; // Return null if no subject found
  };
  ...
  return (
    <div>
      <div className="container"  style={{marginTop: '2rem'}}>
        <table className="table table-light table-bordered text-center" data-intro="To send reschedule request using the module filtered timetable">
          <thead >
            <tr > 
                  ...
                  <tr key={time._id}>
                    ...
                    {days.map((day) => {
                      ...
                      if (subject) {
                        ...
                        return (
                          <td className="tableCell" style={{ background: "#F9FBE7", color: "black" }}
                      ...
                      return (() => {
                        ...
                        return (
                          <td className="tableCell" style={{ background: "#F9FBE7", color: "black" }} key={_.uniqueId()}>
                            {renderCellContent(subject,day,time)}
                          </td>
                        );
                      })();
...
export default LecResTimetable;
