...
export const resAssignLab = async (timetable) => {
  ...
  const filteredLabs = LabsData.filter((Lab) => Lab.capacity >= subject.studentCount && Lab.available === true);


  const response2 = await fetch('http://localhost:3001/api/assignlab/getAssignmentLabs');
  if (!response2.ok) {
  ...
  if (filteredLabs.length === 0) {
     ...
     const overlappingAssignments = assignments.filter(
      ...
      if (overlappingAssignments.length > 0) {
          ...
          try {
              const newLab = await Axios.post('http://localhost:3001/api/assignlab/createAssignmentLab', {
                  labid: Lab.labid,
                  day: day.name,
      ...
      const updatedTimetableResponse = await Axios.get(`http://localhost:3001/api/restimetable/${timetable._id}`);
      ...
      if ((closestLab) && (!updatedTimetable.lectureHall.assigned)) {
          ...
          message = `All available Labs for ${startTime.name} have been assigned to ${subject.subjectCode}`;
      }
  }
...
export const resDeleteAssignLab = async (id) => {
    if(id !== null && id !== "" && id !== " ")
        {
            return await Axios.delete(`http://localhost:3001/api/assignhall/deleteAssignmentLab/${id}`);
        }
}
